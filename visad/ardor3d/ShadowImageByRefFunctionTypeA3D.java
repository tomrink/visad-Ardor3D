...
*/
public class ShadowImageByRefFunctionTypeA3D extends ShadowFunctionTypeA3D {

  private static final long serialVersionUID = 1L;
  private static final int MISSING1 = Byte.MIN_VALUE;      // least byte

  private VisADImageNodeA3D imgNode = null;
  private VisADImageNodeA3D prevImgNode = null;
  ...
  AnimationControlA3D animControl = null;
  ...
  public ShadowImageByRefFunctionTypeA3D(MathType t, DataDisplayLink link, ShadowType parent) 
...
//    }
    ...
    */
	...
	/*private void initRegenFlags(ImageRendererJ3D imgRenderer, ShadowFunctionOrSetType MyAdaptedShadowType,
	 ...
	 //Modified signature of initRegenFlags. Added hasAlpha boolean variable. This variable signifies if alpha channel is present or not.
 	 private void initRegenFlags(ImageRendererA3D imgRenderer, ShadowFunctionOrSetType MyAdaptedShadowType, 
			float constant_alpha, ScalarMap cmap, ScalarMap cmaps[], Data data, DisplayImpl display, 
			float default_values[], float[] value_array, int []valueToScalar, int valueArrayLength, 
  ...
  public boolean doTransform(Object group, Data data, float[] value_array,
    	...
    	if (data.isMissing()) {
      		((ImageRendererA3D) renderer).markMissingVisADBranch();
      		return false;
    	}
    	...
    	if (group instanceof Node && ((Node) group).getNumberOfChildren() > 0) {
       		Node g = (Node) ((Node) group).getChild(0);
        	// WLH 06 Feb 06 - support switch in a branch group.
        	if (g instanceof Node && ((Node) g).getNumberOfChildren() > 0) {
            		reuseImages = true;
        	}
    	}
	...
	if (!adaptedShadowType.getIsTerminal()) {
      		...
      		animControl = (AnimationControlA3D) amap.getControl();
	 ...
	 // check that range is single RealType mapped to RGB only
    	...
    	int color_length;
    	ImageRendererA3D imgRenderer = (ImageRendererA3D) renderer;
    	ImageDataFormat imageType = imgRenderer.getSuggestedBufImageType();
    	if (imageType == ImageDataFormat.BGRA) {
      		color_length = 4;
      		if (!hasAlpha) {
        		color_length = 3;
        		imageType = ImageDataFormat.BGR;
      		}
    	} else if (imageType == ImageDataFormat.BGR) {
      		color_length = 3;
    	} else if (imageType == ImageDataFormat.RG) {
      		color_length = 2;
    	} else if (imageType == ImageDataFormat.Intensity) {
      		color_length = 1;
    	} else {
	...
	if (null != cmap) {
			...
			if (isAlphaConstant(color_table) && !missingTransparent) { //TDR (25JAN2012) If missingTransparent=true, lock this out.
                                if (isColorTableGrey(color_table)) {
                                        imageType = ImageDataFormat.Intensity;
                                        color_length = 1;
	...
	//GHANSHAM: 01MAR2012 GreyScale Texture Support (ends here)
        ...
        if (cmaps != null && missingTransparent) {
           color_length = 4;
           imageType = ImageDataFormat.BGRA;
	...
	regen_colbytes = false;
  	regen_geom = false;
  	apply_alpha = false; 
	initRegenFlags((ImageRendererA3D)renderer, adaptedShadowType, constant_alpha, cmap, cmaps, data, display, default_values, value_array, valueToScalar, valueArrayLength, link, curved_size, hasAlpha);
	if(!reuseImages) {
		regen_geom = true;
		regen_colbytes = true;
		apply_alpha = true;
	...
	//REUSE GEOMETRY/COLORBYTE LOGIC (ENDS HERE)
     	prevImgNode = ((ImageRendererA3D)renderer).getImageNode();
	...
	*/
	if (!reuseImages || (regen_colbytes && regen_geom)) { //REUSE GEOM/COLBYTE:Earlier reuse variable was used. Replaced it with reuseImages.
		...
		Node branch = new Node();
       		...
       		imgNode = new VisADImageNodeA3D();

       		bgImages = new Node();

       		swit.attachChild(bgImages);
       		swit.setSingleVisible(0);
       		branch.attachChild(swit);

       		imgNode.setBranch(branch);
       		imgNode.setSwitch(swit);
       		((ImageRendererA3D)renderer).setImageNode(imgNode);
       		...
       		imgNode = ((ImageRendererA3D)renderer).getImageNode();
       		bgImages = (Node) imgNode.getSwitch().getChild(0);	//REUSE GEOM/COLBYTE:Extract the bgImages from the avaialable switch
     	} 
    ...
    if (null != animControl) {
	SwitchNode swit = new SwitchNotify(imgNode, numImages);  
      	((AVControlA3D) animControl).addPair((SwitchNode) swit, domain_set, renderer);
      	((AVControlA3D) animControl).init();
    }
    ...
    DisplayRenderer dspRenderer = link.getDisplay().getDisplayRenderer();
    com.ardor3d.renderer.Renderer rendererA3D = ((DisplayRendererA3D)dspRenderer).getCanvasRenderer().getRenderer();

    int texture_width_max = dspRenderer.getTextureWidthMax();
    int texture_height_max = dspRenderer.getTextureWidthMax();
	...
	Mosaic mosaic = new Mosaic(data_height, texture_height_max, data_width, texture_width_max);
      	for (Iterator iter = mosaic.iterator(); iter.hasNext();) {
           ...
           imgNode.addTile(new VisADImageTileA3D(rendererA3D, numImages, tile.height, tile.y_start, tile.width, tile.x_start));
      	}
    }
    ...
    if (isTextureMap) { // linear texture
        ...
        if (imgNode.getNumTiles() == 1) {
          VisADImageTileA3D tile = imgNode.getTile(0);
	  if (regen_colbytes) { //REUSE COLBYTES: regenerate only if required
		...
		if (regen_geom) { //REUSE : REGEN GEOM  regenerate the geometry
          		buildLinearTexture(bgImages, domain_set, dataUnits, domain_units, default_values, DomainComponents,
                    ...
                    Node Branch_L1 = (Node) bgImages.getChild(0);
                    /* TODO
	  ...
	  if (!reuseImages) {
		...
		branch = (Node) bgImages.getChild(0);
	   }
	  int branch_tile_indx = 0; //REUSE: to get the branch for a tile in case of multi-tile rendering
          for (Iterator iter = imgNode.getTileIterator(); iter.hasNext();) {
             VisADImageTileA3D tile = (VisADImageTileA3D) iter.next();
		...
		if (regen_geom) { //REUSE: Regenerate the geometry
			...
			if (!reuseImages || (regen_colbytes && regen_geom)) { //REUSE: Make a fresh branch for each tile
				...
				branch1 = (Node) branch.getChild(branch_tile_indx);
			}
			...
			if (!reuseImages|| (regen_colbytes && regen_geom)) {
		        	branch.attachChild(branch1);
			}
			g00 = null;
			g11 = null;
			dset = null;
		} else { //REUSE Reuse the branch fully along with geometry. Just apply the colorbytes(Buffered Image)
                        Node branch1 = (Node) branch.getChild(branch_tile_indx);
                        Node branch2 = (Node) branch1.getChild(0); //Beause we create a branch in textureToGroup
                        /* TODO
		...
		if (0 == branch_tile_indx) { //Add the branch to get rendered as early as possible
                        if (!reuseImages || (regen_colbytes && regen_geom)) { //REUSE : Add a new branch if created
                                if (((Node) bgImages).getNumberOfChildren() > 0) {
                                        ((Node) bgImages).attachChildAt(branch, 0);
                                } else {
                                        ((Node) bgImages).attachChild(branch);
                                }
                        }
                }
	...
	if (!reuseImages || (regen_colbytes && regen_geom)) {  //REUSE: Make a fresh branch
	  ...
	  int branch_tile_indx = 0; //REUSE: to get the branch for a tile in case of multi-tile rendering
		...
		if (regen_geom) { //REUSE REGEN GEOM regenerate geometry 
			...
			Node branch1 = (Node) branch.getChild(branch_tile_indx);
                        Node branch2 = (Node) branch1.getChild(0);
                        /* TODO
                        Shape3D shape = (Shape3D) branch2.getChild(0);
                        applyTexture(shape, tile, apply_alpha, constant_alpha);
                        */
...
//public void makeColorBytesDriverOld(Data imgFlatField, ScalarMap cmap, ScalarMap[] cmaps, float constant_alpha,
//              ShadowRealType[] RangeComponents, int color_length, int domain_length, int[] permute,
//              int data_width, int data_height,
//              int imageType, VisADImageTileA3D tile, int image_index) throws VisADException, RemoteException {
...
public void makeColorBytesDriver(Data imgFlatField, ScalarMap cmap, ScalarMap[] cmaps, float constant_alpha,
              ShadowRealType[] RangeComponents, int color_length, int domain_length, int[] permute,
              int data_width, int data_height,
              ImageDataFormat imgFrmt, VisADImageTileA3D tile, int image_index) throws VisADException, RemoteException {
        byte byteData[] = null;
        int tile_width = tile.width;
        int tile_height = tile.height;
        int xStart = tile.xStart;
        int yStart = tile.yStart;
        int textureWidth = textureWidth(tile_width);
        int textureHeight = textureHeight(tile_height);


       
       if (!reuseImages) {
         if (imgFrmt == ImageDataFormat.BGRA) {
           byteData = new byte[4*textureWidth*textureHeight];
         }
         else if (imgFrmt == ImageDataFormat.BGR) {
           byteData = new byte[3*textureWidth*textureHeight];
         }
         else if (imgFrmt == ImageDataFormat.Intensity) {
           byteData = new byte[textureWidth*textureHeight];
         } 
         
         ByteBuffer bbuf = ByteBuffer.wrap(byteData);
         
         makeColorBytes(imgFlatField, cmap, cmaps, constant_alpha, RangeComponents, color_length, domain_length, permute,
                    byteData, data_width, data_height, tile_width, tile_height, xStart, yStart, textureWidth, textureHeight);
         ...
         //image = (CachedBufferedByteImage) tile.getImage(0);
         Image image = tile.getImage(image_index);
	 //GHANSHAM: 01MAR2012 GreyScale Texture (starts here) 
	 //If the incoming ImageType is not the same as the existing imageType, we will have to recreate the image.
         //THIS HAPPENS when a single band GreyScale Image (GreyScale LUT applied) gets converted Color image(Colored LUT applied) and vice versa
	 if (image.getDataFormat() != imgFrmt) {
            if (imgFrmt == ImageDataFormat.BGRA) {
              byteData = new byte[4*textureWidth*textureHeight];
            }
            else if (imgFrmt == ImageDataFormat.BGR) {
            ...
            image = new Image(imgFrmt, PixelDataType.UnsignedByte, textureWidth, textureHeight, bbuf, null);
            tile.setImage(image_index, image);            
         }
         ByteBuffer bbuf = image.getData(0);
         byteData = bbuf.array();
         makeColorBytes(imgFlatField, cmap, cmaps, constant_alpha, RangeComponents, color_length, domain_length, permute,
                        byteData, data_width, data_height, tile_width, tile_height, xStart, yStart, textureWidth, textureHeight);
	//GHANSHAM: 01MAR2012 GreyScale Texture (ends here)
...
// inside 'if (anyFlow) {}' in ShadowType.assembleSpatial()
	...
	//12NOV2012: GHANSHAM Just adding a general a way to create tile corner coordinates (starts here)
      ...
      */
		...
		int num_children = ((Node) group).getNumberOfChildren();
        	if (num_children > 0) {
                	Node branch1 = (Node) ((Node) group).getChild(0); //This the branch group created by textureToGroup Function
                        /* TODO
    ...
    */                                                                                                               
    // add texture as sub-node of group in scene graph
		...
		int num_children = ((Node) group).getNumberOfChildren();
		if (num_children > 0) {
        		Node branch1 = (Node) ((Node) group).getChild(0); //This is the branch group created by textureToGroup Function
                        /*
                	Shape3D shape = (Shape3D) branch1.getChild(0);
...
class SwitchNotify extends SwitchNode {
  VisADImageNodeA3D imgNode;
  int numChildren;
  SwitchNode swit;
   
  SwitchNotify(VisADImageNodeA3D imgNode, int numChildren) {
    super();
  ...
  public void setSingleVisible(int index) {
     if (index < 0) {
