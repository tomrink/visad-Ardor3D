...
*/
public class RubberBandBoxRendererA3D extends DirectManipulationRendererA3D {
  ...
  public RubberBandBoxRendererA3D (RealType xarg, RealType yarg) {
    this(xarg, yarg, 0, 0);
  }
  ...
  /** xarg and yarg determine spatial ScalarMaps;
      ...
      sub-manifold) */
  public RubberBandBoxRendererA3D (RealType xarg, RealType yarg, int mmm, int mmv) {
    super();
    x = xarg;
    y = yarg;
...
// methods customized from DataRenderer:
  ...
  public synchronized void release_direct() {
    // set data in ref
    if (group != null) branch.detachChild(group);
    group = null;
    try {
...
//        ShadowTypeJ3D.staticMakeAppearance(mode, null, null, geometry, false);
      ...
      if (branch != null) branch.attachChild(group);
    } // end try
    catch (VisADException e) {
      // do nothing
  ...
  public Object clone() {
    return new RubberBandBoxRendererA3D(x, y, mouseModifiersMask,
                                        mouseModifiersValue);
  }
  ...
  public static void createAndShowGUI(String args[])
    ...
    DataReferenceImpl field_ref2 = new DataReferenceImpl("field2");
    field_ref2.setData(field);
    field_ref.setData(field);
    display.addReferences(new DefaultRendererA3D(), field_ref);
    ...
    display.addReferences(new RubberBandBoxRendererA3D(x, y, m, m), ref);
    //display.addReference(field_ref2);
