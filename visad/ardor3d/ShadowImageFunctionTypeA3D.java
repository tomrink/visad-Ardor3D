...
//System.out.println("start curved texture " + (System.currentTimeMillis() - link.start_time));
	...
	//System.err.println("Texture Limit:" + limit);
      ...
      int old_len = 0;
      boolean reuse = ((ImageRendererA3D) renderer).getReUseFrames();
      if (group instanceof Node &&
          ((Node) group).getNumberOfChildren() > 0) {
        Spatial g = ((Node) group).getChild(0);
      ...
      for (int i=0; i<len; i++) {
        if (!mark[i]) {
          ...
          ((ImageRendererA3D) renderer).setVisADBranch(nodes[i]);
          recurseRange(branch, ((Field) data).getSample(i),
                       value_array, default_values, renderer);
          ((ImageRendererA3D) renderer).setVisADBranch(null);
...
// inside 'if (anyFlow) {}' in ShadowType.assembleSpatial()
  ...
  public static void main (String[] args) {
    ...
    try {
      ...
      //--- map data to display ---//
      if (gif) {
        display = new DisplayImplA3D("display",
                                     new TwoDDisplayRendererA3D());
...
/* end modify imaget */
      ...
      display.addReferences(new ImageRendererA3D(), ref_image);
