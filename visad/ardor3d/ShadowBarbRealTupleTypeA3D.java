...
*/
public class ShadowBarbRealTupleTypeA3D extends ShadowRealTupleTypeA3D {

  public ShadowBarbRealTupleTypeA3D(MathType t, DataDisplayLink link,
                                ShadowType parent)
  ...
  public VisADGeometryArray[] makeFlow(int which, float[][] flow_values,
    ...
    if (direct && renderer instanceof BarbManipulationRendererA3D) {
...
// NOTE - must scale to knots
        ...
        float mbarb[] =
          ((BarbRenderer) renderer).makeVector(south[j],
                   ...
                   numv, tx, ty, tz, numt);
        if (direct) {
          ((BarbManipulationRendererA3D) renderer).
