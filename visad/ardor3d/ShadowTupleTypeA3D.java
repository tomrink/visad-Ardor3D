...
/**
   The ShadowTupleTypeA3D class shadows the TupleType class,
   within a DataDisplayLink.<P>
*/
public class ShadowTupleTypeA3D extends ShadowTypeA3D {

  ShadowTypeA3D[] tupleComponents;
  private Vector AccumulationVector = new Vector();

  public ShadowTupleTypeA3D(MathType t, DataDisplayLink link,
                            ShadowType parent)
         throws VisADException, RemoteException {
    super(t, link, parent);
    if (this instanceof ShadowRealTupleTypeA3D) return;

    int n = ((TupleType) t).getDimension();
    tupleComponents = new ShadowTypeA3D[n];
    ShadowType[] components = new ShadowType[n];
    for (int i=0; i<n; i++) {
      ShadowTypeA3D shadow = (ShadowTypeA3D)
  ...
  public boolean recurseComponent(int i, Object group, Data data,
    ...
    return tupleComponents[i].doTransform(new_node, data, value_array,
