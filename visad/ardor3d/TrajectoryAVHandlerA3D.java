 ...
 * @author rink
 */
public class TrajectoryAVHandlerA3D implements AVHandler {
    int numChildren;
    int[] whichVisible;
    int numVisible;
    int direction;
   ...
   public TrajectoryAVHandlerA3D(SwitchNode swit, int numChildren, int[] whichVisible, int direction) {
     this.swit = swit;
   ...
   public void setWhichChild(int index) {
      if (index == VisADSwitch.NONE_VISIBLE) {
        bits.clear();
        swit.setVisible(bits);
      }
      else if (index >= 0) {
        bits.clear();
